...
<template>
  ...
  >
    ...
    <p
      ...
      class="analysis-note"
    >
      Saved analysis covers {{ formatCount(coverage.left.reviewed) }} of {{ formatCount(left.length) }} entries in the first period and {{ formatCount(coverage.right.reviewed) }} of {{ formatCount(right.length) }} in the second. Tags reflect the information in each logline; sparse descriptions may leave a pattern unestablished.
    </p>
    <p
      v-if="first === second"
      class="analysis-note"
    ...
    <p
      v-if="!left.length || !right.length"
      class="analysis-note"
      role="status"
    >
      At least one period has no entries under these filters. Widen the year range or change the periods to compare their shares.
    </p>
    <div
    ...
    >
      <table class="analysis-comparison-table">
        <caption class="visually-hidden">
          {{ basis === 'genre' ? 'Genre' : 'Tagged pattern' }} shares within filtered archive entries. Change is in percentage points.
        </caption>
        <thead>
          <tr>
            <th scope="col">
        ...
        <tbody>
          ...
          >
            ...
            <td
              v-for="(cell, index) in [row.first, row.second]"
              :key="index"
            >
              <button
                type="button"
              ...
              >
                {{ (index === 0 ? left.length : right.length) ? `${cell.share}%` : '—' }}<small>{{ formatCount(cell.count) }} entries</small>
              </button>
            </td>
    ...
    <p class="analysis-note">
      {{ basis === 'genre' ? 'Exact source genre labels are kept intact. Shares use all matching entries in each period, including those with no genre listed.' : 'An entry can have several tags. Untagged entries are included in the denominator.' }} Archive coverage is uneven; these are not market-wide trends.
    </p>
  </section>
</template>
