  ...
  <main class="analysis-page analysis-explorer">
    <header class="analysis-header">
      <div class="analysis-header-nav">
        <NuxtLink to="/" class="analysis-back-link">
      ...
      <p class="analysis-intro">
        Explore {{ formatCount(deals.length) }} recorded spec script deals. Follow a story pattern, compare periods, and
        read the loglines behind the numbers.
      </p>
    </header>
    ...
    <div v-else class="analysis-content">
      <div class="analysis-filter-bar">
        <div class="grid grid-cols-2 gap-4 md:grid-cols-[1fr_1fr_2fr_auto] md:items-end">
          <UFormField label="From year" name="from-year">
            <USelect v-model="fromYear" :items="years" class="w-full"
              :ui="{ base: 'analysis-select', content: 'analysis-select-menu' }" />
          </UFormField>
          <UFormField label="To year" name="to-year">
            <USelect v-model="toYear" :items="years" class="w-full"
              :ui="{ base: 'analysis-select', content: 'analysis-select-menu' }" />
          </UFormField>
          <UFormField label="Genre" name="genre" class="col-span-2 md:col-span-1">
            <USelectMenu v-model="genre" aria-label="Genre" :items="genres" value-key="value" class="w-full"
              :ui="{ base: 'analysis-select', content: 'analysis-select-menu', input: 'w-full' }" />
          </UFormField>
          <UButton color="neutral" variant="ghost" :disabled="!isFiltered" @click="resetFilters">
            Reset filters
          </UButton>
        </div>
        <p class="analysis-filter-summary" role="status">
          <strong>{{ formatCount(analysis.total) }}</strong> of {{ formatCount(deals.length) }} entries · {{ fromYear
          }}–{{ toYear }}
      ...
      <section id="patterns" class="analysis-section" aria-labelledby="patterns-heading">
        ...
        <div class="analysis-pattern-layout">
          <AnalysisBarList :rows="patterns" :limit="8" @select="showPattern" />
          <aside class="analysis-pattern-note">
            <h3>Saved analysis</h3>
            <p><strong>{{ formatCount(coverage.reviewed) }} of {{ formatCount(analysis.total) }}</strong> loglines
              analysed in this selection. Coverage: {{ percentage(coverage.reviewed, analysis.total) }}%.</p>
            <p>{{ formatCount(coverage.tagged) }} entries match at least one recurring story motif. {{
              formatCount(coverage.withoutPattern) }} have no established match.</p>
            <p v-if="coverage.limited">
              {{ formatCount(coverage.limited) }} entries contain no usable logline or too little story detail to
              extract a protagonist, goal, obstacle or stakes.
            </p>
            <p>Every analysis is saved and reused. One story can have several tags; no match does not mean it was
              skipped.</p>
            <UButton color="neutral" variant="outline" :disabled="!coverage.reviewed" @click="showAnalysedEntries">
              Browse analysed loglines
            </UButton>
            <a href="#method">How the analysis was made
              <UIcon name="i-lucide-arrow-down" aria-hidden="true" />
            </a>
          </aside>
        </div>
        <p v-if="!coverage.tagged" class="analysis-note">
          No story patterns have been established for this selection. The source-data charts below still include every
          matching entry.
        </p>
        <p class="analysis-note">
          Percentages use all {{ formatCount(analysis.total) }} filtered entries. Open any row to inspect the source
          loglines and tag definitions.
        </p>
      </section>

      <AnalysisPeriodComparison v-model:first="periodA" v-model:second="periodB" v-model:basis="comparisonBasis"
        :periods="periods" :rows="comparison" :left="leftPeriod" :right="rightPeriod" :coverage="periodCoverage"
        @select="showDeals" />
      ...
      <section id="buyers" class="analysis-section" aria-labelledby="buyers-heading">
        ...
        <p class="analysis-note">
          Top 10 recorded names. {{ formatCount(buyerCoverage) }} of {{ formatCount(analysis.total) }} entries list a {{
            buyerField === 'studio' ? 'studio' : 'production company' }}. Original names and joint credits stay intact;
          these are historical credits, not current buying preferences.
        </p>
      </section>
      ...
      <section id="archive-data" class="analysis-section" aria-labelledby="years-heading">
        <div class="analysis-section-heading">
          ...
          <span v-if="peakYear?.count" class="analysis-source-label">Peak: {{ peakYear.label }} · {{ peakYear.count }}
            entries</span>
        </div>
      ...
      <div class="analysis-grid">
        <section class="analysis-section" aria-labelledby="genres-heading">
          ...
          <p class="analysis-note">
            Combined labels stay intact. {{ missingGenres }} entries have no recorded genre. Percentages include those
            entries.
          </p>
        </section>
      ...
      <section class="analysis-section" aria-labelledby="words-heading">
        ...
        <p class="analysis-note">
          Common function words are excluded. “Find” and “finds” remain separate. Word frequency is not a story-pattern
          classification.
        </p>
      </section>
    </div>

    <footer id="method" class="analysis-footer analysis-method">
      <div>
        <h2>About this analysis</h2>
        <p>Counts are calculated from the Myers Archive dataset. Its {{ formatCount(deals.length) }} extracted entries
          include two more than the source’s stated count of 2,560. Entries are counted as recorded, including possible
          repeat reports.</p>
        <details>
          <summary>AI tags, coverage &amp; limitations</summary>
          <p>{{ storyReviewMethod }}</p>
          <p>Saved analysis: {{ storyReviewDate }}. Source genres and deal details are preserved. An edited logline
            keeps its previous analysis on file, but that analysis is excluded from the charts until refreshed. {{
              coverage.stale }} changed and {{ coverage.missing }} new entries need analysis in this selection.</p>
          <p>The review uses only the logline, not the full screenplay. It does not establish production outcomes,
            commercial success, or a script’s likelihood of selling.</p>
        </details>
      </div>
      <NuxtLink to="/" class="analysis-footer-link">Return to archive
